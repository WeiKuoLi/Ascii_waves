...
map<double, char> create_map(vector<double> nums, string str) {
  int len, i;
  map<double, char> char_map;
  len = min(nums.size(), str.size());
  for (i = 0; i < len; i++) {
    char_map[nums[i]] = str[i];
  }
  return char_map;
}
...
string render_image(vector<vector<double>> *mp) {
  ...
  vector<string> ascii_result(n, "");
  int i, j;
  for (i = 0; i < n; i++) {
    for (j = 0; j < m; j++) {
      auto closest = ascii_map.lower_bound((*mp)[i][j]);
      ascii_result[i] += ' ';
      ascii_result[i] += closest->second;
    }
    image += ascii_result[i] + "\r\n";
  }
...
vector<vector<double>> *make_waves(int n, int m, double k, double w, double t) {
  vector<vector<double>> *mp =
      new vector<vector<double>>(n, vector<double>(m, 0.0));
  int i, j;
